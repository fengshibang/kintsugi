...
6. 集成徒弟逐个 `git merge <branch-N>` 到 <仓库默认分支> 合并所有 part + 解决冲突 + 跑通整体
7. 师傅对集成徒弟的产出做最终三层审查
...
【worktree】先 `cd <worktree 路径>` 再开始工作。你工作在独立 worktree（<worktree 路径>），分支 <part-N>，改动只在本 worktree 的该分支，不要动 <仓库默认分支> 分支
...
```
【任务】将以下 part 的改动合并到 <仓库默认分支> 分支，解决冲突，确保整体跑通
【part 列表】
...
- **清理**：师傅在集成徒弟完成后 `git worktree remove <path>` 清理所有 part worktree，并删除已合并的 part 分支（`git branch -d <part-N>`）
