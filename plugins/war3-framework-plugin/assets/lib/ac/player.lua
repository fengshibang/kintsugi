...
function mt:getSelectedUnit()
    return ac.unit(japi.GetPlayerSelectedUnit(self.handle))
end
...
function mt:is_player()
    return jass.GetPlayerController(self.handle) == jass.MAP_CONTROL_USER and jass.GetPlayerSlotState(self.handle) ==
...
function mt:addGold(gold, where, flag)
    ...
    jass.SetPlayerState(self.handle, jass.PLAYER_STATE_RESOURCE_GOLD, self.gold)
    ...
    local x, y = where:getPoint():get()
    local z = where:getpoint():getZ()
    local position = Types.point(x, y, z + 30)
    ac.texttag {
...
function mt:addLumber(lumber, where, flag)
    if not lumber then
        return
    end
    ...
    self:addlumber(lumber)
    self.lumber = self.lumber + lumber
    if not where or lumber <= 0 then
        return
    end
    if where.type == 'unit' then
        if not where:is_visible(self) then
...
function mt.addlumber(self, lumber)
    jass.SetPlayerState(self.handle, jass.PLAYER_STATE_RESOURCE_LUMBER,
        jass.GetPlayerState(self.handle, jass.PLAYER_STATE_RESOURCE_LUMBER) + lumber)
end
function mt.addusedfood(self, food)
    jass.SetPlayerState(self.handle, jass.PLAYER_STATE_RESOURCE_FOOD_USED,
        jass.GetPlayerState(self.handle, jass.PLAYER_STATE_RESOURCE_FOOD_USED) + food)
end
...
function mt:cinematic_filter(data)
    ...
    if data.start then
        jass.SetCineFilterStartColor(data.start[1] * 2.55, data.start[2] * 2.55, data.start[3] * 2.55,
            data.start[4] * 2.55)
    end
    if data.finish then
        jass.SetCineFilterEndColor(data.finish[1] * 2.55, data.finish[2] * 2.55, data.finish[3] * 2.55,
            data.finish[4] * 2.55)
    end
...
function player:__call(i)
    if type(i) == "userdata" then
        return player[GetConvertedPlayerId(i)]
    else
        return player[i]
    end
end
...
function player.regist_jass_triggers()
    ...
    local trg = war3.CreateTrigger(function()
		...
		if p == player.master then
			for i = 1, 16 do
				if player[i]:is_player() then
					player.master = player[i]
					break
				end
			end
		end
...
function player.regist_hardware_triggers()
...
local function init()
	...
	player.regist_hardware_triggers()
    -- 注册选取事件
