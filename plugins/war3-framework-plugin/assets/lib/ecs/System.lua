...
function System:addEntity(entity, category)
    ...
    if category then
        self.targets[category][entity.eid] = entity
    else
        -- Otherwise they'll be added to the normal self.targets list
        self.targets[entity.eid] = entity
    end
...
function System:removeEntity(entity, group)
    if group and self.targets[group][entity.eid] then
        self.targets[group][entity.eid] = nil
    ...
    if firstGroup then
        if self.hasGroups then
            ...
            for group, _ in pairs(self.targets) do
                if self.targets[group][entity.eid] then
                    self.targets[group][entity.eid] = nil
                    self:onRemoveEntity(entity, group)
                end
            end
        else
            if self.targets[entity.eid] then
                self.targets[entity.eid] = nil
...
function System:pickRequiredComponents(entity)
    ...
    if type(ecs.util.firstElement(requirements)) == "string" then
        for _, componentName in pairs(requirements) do
            table.insert(components, entity:get(componentName))
        end
    elseif type(ecs.util.firstElement(requirements)) == "table" then
