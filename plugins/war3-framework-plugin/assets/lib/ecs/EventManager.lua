...
local EventManager = ecs.class("EventManager")
...
function EventManager:addListener(eventName, listener, listenerFunction)
    ...
    if not listener.class or (listener.class and not listener.class.name) then
        ecs.debug('Eventmanager: The listener has to implement a listener.class.name field.')
    end
    ...
    if type(listenerFunction) == 'function' then
        ...
        ecs.debug('Eventmanager: Third parameter has to be a function! Please check listener for ' .. eventName)
        if listener.class and listener.class.name then
            ecs.debug('Eventmanager: Listener class name: ' .. listener.class.name)
        end
    end
end
...
function EventManager:removeListener(eventName, listener)
    if self.eventListeners[eventName] then
        ...
        ecs.debug(string.format("Eventmanager: Listener %s to be deleted on Event %s  is not existing.", listener.class.name, eventName))
    end
    ecs.debug(string.format("Eventmanager: Event %s listener should be removed from is not existing ", eventName))
end
