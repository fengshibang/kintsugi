...
local function InScren(screen_x, screen_y, screen_z)
    return not (screen_x == nil or screen_x < 0 or screen_y < 32 or screen_x > 1920 or screen_y > 820)
end

local CenterScrenX, CenterScrenX_Div = 1920 / 2, 35
local function ReviseScrenX(x)
    if x < CenterScrenX then
        return x - CenterScrenX_Div * ((CenterScrenX - x) / CenterScrenX)
    elseif x > CenterScrenX then
        return x + CenterScrenX_Div * ((x - CenterScrenX) / CenterScrenX)
    end
    return x
end
...
local function get_handle_type(handle)
    ...
    if jass.GetHandleId(handle) == 0x100000 then
        return nil
    end

    local retval
    jass.RemoveSavedHandle(ht, 1, 1)
    jass.SaveFogStateHandle(ht, 1, 1, handle)
    if jass.LoadItemHandle(ht, 1, 1) ~= nil and jass.LoadItemHandle(ht, 1, 1) ~= 0 then
        retval = 1
    elseif jass.LoadUnitHandle(ht, 1, 1) ~= nil and jass.LoadUnitHandle(ht, 1, 1) ~= 0 then
        retval = 2
    end
    jass.RemoveSavedHandle(ht, 1, 1)
    return retval
...
base = {
    ...
    on_mouse_down = function()
        ...
        local button = ui_class.button.button_map[id]

        left_is_down = true
        if button == nil then
            for id, btn in pairs(ui_class.button.button_map) do
            ...
            if button.is_drag == true and button.is_enable then
                ...
                game.wait(0.3, function()
                    if left_is_down == true then
                        ...
                        texture = ui_class.texture:builder
                        {
                            x = x - button.w / 2,
                            y = y - button.h / 2,
                            w = width,
    ...
    on_mouse_move = function()
        ...
        for id, button in pairs(ui_class.button.button_map) do
    ...
    on_mouse_wheeldelta = function()
        ...
        for id, panel in pairs(ui_class.panel.panel_map) do
    ...
    on_update = function()
        ...
        for control in pairs(world_controls) do
            ...
            if not InScren(screen_x, screen_y, scale) then
                control:hide()
            else
                control:show()
                local x = ReviseScrenX(screen_x - control.w + (control.offect_x or 0))
...
local frame_event = {
    --文本框更新事件
    [9] = function(frame, id)
        local edit = ui_class.edit.edit_map[frame]
        if edit == nil then
            return
        end
