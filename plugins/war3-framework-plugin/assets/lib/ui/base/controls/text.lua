...
ui_class.text = extends(ui_class.panel){
--private
...
--static
    ...
    --构建
    build = function (self)
        self.align = ui_class.text.align_map[self.align or ''] or self.align or 0

       -- local panel = ui_class.panel.new(self.parent,self.normal_image, 0, 0, self.w, self.h)
    ...
    new = function (parent,text,x,y,width,height,font_size,align)
        local control = ui_class.text:builder
        {
            parent = parent,
            text = text,
            x = x,
    ...
    set_width = function (self, width)
        ui_class.panel.set_width(self, width)
        self._panel:set_width(width)
    end,

    set_height = function (self, height)
        ui_class.panel.set_height(self, height)
    ...
    set_control_size = function (self,width,height)
        ...
        self.h = height
        if self.align == -1 then 
            ui_class.panel.set_control_size(self,width,height)
        end
