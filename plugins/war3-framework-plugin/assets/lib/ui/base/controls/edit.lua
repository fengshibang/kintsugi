...
ui_class.edit = extends(ui_class.panel){
    ...
    build = function (self)
        ...
        local panel = ui_class.panel.new(self.parent, '', 0, 0, self.w, self.h)

        if panel == nil then 
            ecs.debugError('文本框背景创建失败')
            return
        end
        panel._control = self
        self._panel = panel
    ...
    new = function (parent, text, x, y, width, height, font_size)
        local control = ui_class.edit:builder
        {
            parent = parent,
            text = text,
            x = x,
    ...
    destroy = function (self)
        ...
        ui_class.ui_base.destroy(self._index)
    end,
