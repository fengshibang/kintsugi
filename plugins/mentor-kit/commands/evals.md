---
description: 跑eval回归看通过率/baseline delta/规范库增益
argument-hint: [领域filter 如 buff/system/migrate; 留空跑全部]
allowed-tools: ["Bash(${CLAUDE_PLUGIN_ROOT}/framework/*:*)", "Read"]
---

跑 eval 回归，量化规范库（硬红线 + rubric）覆盖率是否 ↑。参数：`$ARGUMENTS`（领域 filter；留空跑全部 case）。
...
2. 跑完 Read `$PWD/.claude/evals/report/report.md` 和 `report/summary.json`，汇报：
   ...
   - 每 case 的 **baseline delta**（normal − baseline：>0 = 规范库正向增益；<0 = 回归）
   - 截断 / runner·judge 失败的 case
   - CI 门禁 PASS / FAIL

3. 若通过率低于阈值或有回归 case：分析是规范库退化还是 case 内容问题，给改进建议（补硬红线 / 改 rubric）。
